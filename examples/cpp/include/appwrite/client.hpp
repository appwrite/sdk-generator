...
public:
    ...
    [[nodiscard]] std::unordered_map<std::string, std::string> getHeaders() const { return get_cfg()->headers; }
    ...
    Result<T> call(std::string m, std::string p, 
        ...
        } catch (const AppwriteException&) {
            return Result<T>::Err(std::current_exception());
        } catch (const std::exception& e) {
            return Result<T>::Err(std::make_exception_ptr(DeserializationException(e.what())));
        }
    }
    ...
    Result<void> callVoid(std::string m, std::string p,
        ...
        try {
            ...
            return Result<void>::Err(std::current_exception());
        } catch (const std::exception& e) {
            return Result<void>::Err(std::make_exception_ptr(DeserializationException(e.what())));
        }
    }
    ...
    Result<std::string> callLocation(std::string m, std::string p, 
        ...
        try {
            ...
            return Result<std::string>::Err(std::current_exception());
        } catch (const std::exception& e) {
            return Result<std::string>::Err(std::make_exception_ptr(AppwriteException(e.what())));
        }
    }
    ...
    Result<BinaryResponse> callBytes(std::string m, std::string p, 
        ...
        try {
            ...
            return Result<BinaryResponse>::Err(std::current_exception());
        } catch (const std::exception& e) {
            return Result<BinaryResponse>::Err(std::make_exception_ptr(AppwriteException(e.what())));
        }
    }
    ...
    Result<T> fileUpload(std::string m, std::string p, std::string fk,
        ...
        try {
            ...
            return Result<T>::Err(std::make_exception_ptr(AppwriteException(e.what())));
        }
    }
...
// never reached the server) to avoid duplicate side effects.
    ...
    static cpr::Multipart prepareMultipart(const nlohmann::json& pms) {
        ...
        for (auto const& [key, val] : pms.items()) {
            if (val.is_string()) {
                multipart.parts.emplace_back(key, val.template get<std::string>());
                ...
                multipart.parts.emplace_back(key, val.dump());
            }
        }
        return multipart;
    }
    ...
    static Task<T> makeResolvedTask(std::future<T> fut) {
        ...
        return [](std::shared_ptr<std::future<T>> f) -> Task<T> {
            try {
                ...
                co_return T::Err(std::current_exception());
            } catch (const std::exception& e) {
                co_return T::Err(std::make_exception_ptr(AppwriteException(e.what())));
            }
        }(std::move(shared));
    }
};
