...
class Auth extends Service {
    ...
    login({email, password, success = null, failure = null}) async {
    ...
    oauthCallback({projectId, provider, code, state = null}) async {
    ...
    oauth({provider, success = null, failure = null}) async {
    ...
    recovery({email, redirect}) async {
     ...
     /// the only valid redirect URL&#039;s are the once from domains you have set when
     /// added your platforms in the console interface.
    recoveryReset({userId, token, passwordA, passwordB}) async {
     ...
     /// clients deal with browsers who don&#039;t allow to set 3rd party HTTP cookies
     /// needed for saving the account session token.
    register({email, password, redirect, name = null, success = null, failure = null}) async {
