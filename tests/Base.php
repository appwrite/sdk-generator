...
abstract class Base extends TestCase
{
    ...
    protected function assertPhpTemplatedModelGenerationWorks(): void
    {
        $spec = file_get_contents(realpath(__DIR__ . '/resources/spec.json'));

        if ($spec === false || $spec === '') {
            throw new \RuntimeException('Failed to read base spec fixture.');
        }

        $data = json_decode($spec, true, 512, JSON_THROW_ON_ERROR);

        $data['paths']['/mock/tests/general/templated-models'] = [
            'post' => [
                'summary' => 'Create Enum Payload',
                'operationId' => 'generalCreateEnumPayload',
                'consumes' => ['application/json'],
                'produces' => ['application/json'],
                'tags' => ['general'],
                'description' => 'Fixture-only method used to generate enum-heavy request and response models.',
                'responses' => [
                    '200' => [
                        'description' => 'Enum result',
                        'schema' => [
                            '$ref' => '#/definitions/enumResult',
                        ],
                    ],
                ],
                'x-appwrite' => [
                    'method' => 'createEnumPayload',
                    'weight' => 302,
                    'cookies' => false,
                    'type' => '',
                    'demo' => 'general/create-enum-payload.md',
                    'edit' => 'https://github.com/appwrite/appwrite/edit/masterCreate enum-heavy fixture models.',
                    'rate-limit' => 0,
                    'rate-time' => 3600,
                    ...
                    [
                        ...
                        'schema' => [
                            ...
                            'properties' => [
                                'payload' => [
                                    'type' => 'object',
                                    'description' => 'Enum-heavy payload object.',
                                    'x-model' => 'enumPayload',
                                    'default' => null,
                                ],
                            ],
        ...
        $data['definitions']['enumPayload'] = [
            'description' => 'Enum payload',
            'type' => 'object',
            'x-request-model' => true,
            'properties' => [
                'default' => [
                    'type' => 'string',
                    'description' => 'Default status label.',
                    'x-example' => 'active',
                ],
                'status' => [
                    'type' => 'string',
                    'description' => 'Primary fixture status.',
                    'x-example' => 'active',
                    'enum' => ['active', 'inactive', 'pending'],
                    'x-enum-name' => 'FixtureStatus',
                ],
                'captain' => [
                    '$ref' => '#/definitions/player',
                    'description' => 'Fixture captain.',
                ],
                'statuses' => [
                    'type' => 'array',
                    'description' => 'Fixture status history.',
                    'items' => [
                        'type' => 'string',
                        'enum' => ['active', 'inactive', 'pending'],
                        'x-enum-name' => 'FixtureStatus',
                ...
                'history' => [
                    'type' => 'array',
                    'description' => 'Fixture history state changes.',
                    'items' => [
                        'type' => 'string',
            ...
            'required' => ['status', 'captain', 'statuses', 'history', 'requestOnlyHistory', 'players'],
        ];

        $data['definitions']['enumResult'] = [
            'description' => 'Enum result',
            'type' => 'object',
            ...
            'required' => ['status', 'captain', 'statuses', 'history', 'players'],
        ];

        $namespace = 'appwrite_regression_' . strtolower((new \ReflectionClass($this))->getShortName());
        $sdk = new SDK(new Language\PHP(), new Swagger2(json_encode($data, JSON_THROW_ON_ERROR)));

        $sdk
            ->setName('php-regression')
            ->setVersion('0.0.1')
            ->setPlatform('server')
            ->setDescription('Regression fixture')
            ->setShortDescription('Regression fixture')
            ->setLogo('https://appwrite.io/v1/images/console.png')
            ->setWarning('Regression fixture')
            ->setExamples('Regression fixture')
            ->setGitUserName('repoowner')
            ->setGitRepoName('reponame')
            ->setLicense('BSD-3-Clause')
            ->setLicenseContent('demo license')
        ...
        $dir = __DIR__ . '/sdks/' . strtolower((new \ReflectionClass($this))->getShortName()) . '-php-regression';
        $this->rmdirRecursive($dir);
        $sdk->generate($dir);

        $resolvedNamespace = implode('\\', array_map(
            static fn (string $segment): string => str_replace(' ', '', ucwords(str_replace(['_', '-'], ' ', $segment))),
            explode('\\', $sdk->getParam('namespace'))
        ));
        $namespacePath = implode('/', array_map(
            static fn (string $segment): string => str_replace(' ', '', ucwords(str_replace(['_', '-'], ' ', $segment))),
            explode('\\', $sdk->getParam('namespace'))
        ));
        $root = $dir . '/src/' . $namespacePath;

        $this->assertFileExists($root . '/Enums/FixtureStatus.php');
        $this->assertFileExists($root . '/Enums/FixtureHistoryStatus.php');
        $this->assertFileExists($root . '/Enums/FixtureRequestHistoryStatus.php');
        $this->assertFileExists($root . '/Models/Player.php');
        $this->assertFileExists($root . '/Models/EnumPayload.php');
        $this->assertFileExists($root . '/Models/EnumResult.php');

        foreach (glob($root . '/Enums/*.php') ?: [] as $file) {
            require_once $file;
        }
        ...
        $enumPayloadClass = $resolvedNamespace . '\\Models\\EnumPayload';
        $enumResultClass = $resolvedNamespace . '\\Models\\EnumResult';
        $playerClass = $resolvedNamespace . '\\Models\\Player';
        $fixtureStatusClass = $resolvedNamespace . '\\Enums\\FixtureStatus';
        $fixtureHistoryClass = $resolvedNamespace . '\\Enums\\FixtureHistoryStatus';
        $fixtureRequestHistoryClass = $resolvedNamespace . '\\Enums\\FixtureRequestHistoryStatus';

        $payload = $enumPayloadClass::from([
            'default' => 'active',
            'status' => 'active',
        ...
        $this->assertSame($fixtureStatusClass, get_class($payload->status));
        $this->assertSame($fixtureStatusClass, get_class($payload->statuses[0]));
        $this->assertSame($fixtureHistoryClass, get_class($payload->history[0]));
        $this->assertSame($fixtureRequestHistoryClass, get_class($payload->requestOnlyHistory[0]));
        $this->assertSame($playerClass, get_class($payload->captain));
        $this->assertSame($playerClass, get_class($payload->players[0]));
        $this->assertSame([
            'default' => 'active',
            'status' => 'active',
            'captain' => ['id' => 'captain1', 'name' => 'Captain One', 'score' => 300],
        ...
        $inactive = $fixtureStatusClass::INACTIVE();
        $result = $enumResultClass::from([
            'default' => 'inactive',
            'status' => 'pending',
            ...
            'custom' => $inactive->jsonSerialize(),
            'nested' => ['enabled' => true],
        ]);

        $this->assertSame($fixtureStatusClass, get_class($result->status));
        $this->assertSame($fixtureStatusClass, get_class($result->statuses[0]));
        $this->assertSame($fixtureHistoryClass, get_class($result->history[0]));
        $this->assertSame($playerClass, get_class($result->captain));
        $this->assertSame($playerClass, get_class($result->players[0]));
        $this->assertSame([
            'default' => 'inactive',
            'status' => 'pending',
            'captain' => ['id' => 'captain2', 'name' => 'Captain Two', 'score' => 400],
